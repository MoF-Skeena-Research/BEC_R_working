...
#type = "Zonal"
# vsum = vegSum; spp=taxon.lifeform; min.cons = 25; strata.by = "Layer"; type = "numeric"
# file_name = "AA_D_upland_units2"; file_path = "./vegsum.tables/"

create_unit_vegtables <- function(
    vsum = vegSum,
    spp = taxon.all,
    type = "BGC",
    min.cons = 33,
  ...
  unit_tables <- lapply(unit_cols, function(unit) {
    ...
    if (strata.by == "Lifeform") {
      ...
      df_unit <- vsum2 %>%
        ...
        filter(Constancy >= min.cons)   # <-- NEW FILTER
      
      
      # Set correct ecological order
      desired_order <- lifeform_tbl$LifeformName
      present_levels <- intersect(desired_order, unique(df_unit$LifeformName))
      
      df_unit$LifeformName <- factor(df_unit$LifeformName,
                                     levels = present_levels,
                                     ordered = TRUE)
      
      # Now order rows correctly
      df_unit <- df_unit %>%
        arrange(LifeformName, desc(Constancy), desc(MeanCover)) %>%

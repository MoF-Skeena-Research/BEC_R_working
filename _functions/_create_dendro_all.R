create_dendro_all <- function(unit.compare,
  ...
  # Only build & show plot if requested
  if (output) {
    ...
    yy <- ggplot() +
      ...
      # Value labels for thresholds
      geom_text(aes(x = 0, y = (threshold.low + 0.02), 
                    label = paste0((threshold.low * 100), "%"), 
                    hjust = 0), angle = 90, color = "red", size = 3) +
      geom_text(aes(x = 0, y = (cut.level + 0.02), 
                    label = paste0((cut.level * 100), "%"), 
                    hjust = 0), angle = 90, color = "darkgreen", size = 3) +
      
      labs(
        title = "Cluster Dendrogram of Site Units",
        subtitle = paste0(
          "Cophenetic Correlation: ", round(cophenetic, 3),
...
# 
# create_dendro_all <- function(unit.compare, threshold.low = .1, cut.level = .2, method = "average", parameter = 1, output = TRUE){
#   compared <- unit.compare 
#   dis.matrix <- bec_dist_matrix(compared) 
#   ss_clst <- agnes(dis.matrix,
